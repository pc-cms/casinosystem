...
export const DEFAULT_CHIP_HEX: Record<number, { bg: string; text: string }> = {
  500:        { bg: "#DC2626", text: "#FFFFFF" }, // red-600
  1_000:      { bg: "#2563EB", text: "#FFFFFF" }, // blue-600
  2_000:      { bg: "#16A34A", text: "#FFFFFF" }, // green-600
  5_000:      { bg: "#9333EA", text: "#FFFFFF" }, // purple-600
  10_000:     { bg: "#EAB308", text: "#000000" }, // yellow-500
  25_000:     { bg: "#F97316", text: "#FFFFFF" }, // orange-500
  50_000:     { bg: "#DB2777", text: "#FFFFFF" }, // pink-600
  100_000:    { bg: "#000000", text: "#FFFFFF" },
  500_000:    { bg: "#0D9488", text: "#FFFFFF" }, // teal-600
  1_000_000:  { bg: "#FBBF24", text: "#000000" }, // amber-400
  5_000_000:  { bg: "#BE123C", text: "#FFFFFF" }, // rose-700
};
...
export const useChipColors = () => {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<Record<number, { bg: string; text: string }>> => {
      if (!casinoId) return {};
      const { data, error } = await supabase
        .from("chip_color_settings")
        .select("denomination, bg_color, text_color")
        .eq("casino_id", casinoId);
      if (error) throw error;
      const map: Record<number, { bg: string; text: string }> = {};
      (data || []).forEach((r: any) => {
        map[Number(r.denomination)] = { bg: r.bg_color, text: r.text_color };
      });
      return map;
    },
    enabled: !!casinoId,
...
export const resolveChipColor = (
  denom: number,
  overrides?: Record<number, { bg: string; text: string }>,
) => {
  if (overrides?.[denom]) return overrides[denom];
  return DEFAULT_CHIP_HEX[denom] || { bg: "#6B7280", text: "#FFFFFF" };
};
...
export const useUpsertChipColor = () => {
  ...
  return useMutation({
    mutationFn: async (input: { denomination: number; bg_color: string; text_color: string }) => {
