...
const UsersAndRoles = () => {
  ...
  const assignRole = useMutation({
    mutationFn: async ({ userId, role }: { userId: string; role: string }) => {
      ...
      if (error) throw error;
      if (activeCasinoId) await logAction(activeCasinoId, "system", "ROLE_ASSIGNED", { user_id: userId, role });
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["all-user-roles"] });
  ...
  const removeRole = useMutation({
    mutationFn: async ({ userId, role }: { userId: string; role: string }) => {
      ...
      if (error) throw error;
      if (activeCasinoId) await logAction(activeCasinoId, "system", "ROLE_REMOVED", { user_id: userId, role });
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["all-user-roles"] });
            ...
            {profiles.map(profile => {
              ...
              return (
                <tr key={profile.user_id} className="border-b border-border last:border-0">
                  ...
                  <td className="px-4 py-3 text-xs font-mono text-muted-foreground">{profile.casino_id?.slice(0, 8)}</td>
                  ...
                  <label key={role} className="flex items-center gap-2 text-sm cursor-pointer">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => {
                        setNewRoles(prev => checked ? [...prev, role] : prev.filter(r => r !== role));
                      }}
                    />
                    {ROLE_LABELS[role]}
                  </label>
