          ...
          return (
            <div key={table.id} className="cms-panel">
              ...
              <div className="px-4 py-3 grid grid-cols-3 gap-3">
                <div><p className="text-[10px] uppercase text-muted-foreground tracking-wider">Float</p><p className="font-mono text-sm font-bold text-card-foreground">€{Number(table.float_amount).toLocaleString()}</p></div>
                <div><p className="text-[10px] uppercase text-muted-foreground tracking-wider">Drop</p><p className="font-mono text-sm font-bold cms-amount-negative">€{totalDrop.toLocaleString()}</p></div>
                <div><p className="text-[10px] uppercase text-muted-foreground tracking-wider">Win</p><p className={`font-mono text-sm font-bold ${totalDrop - totalCashout >= 0 ? "cms-amount-positive" : "cms-amount-negative"}`}>€{Math.abs(totalDrop - totalCashout).toLocaleString()}</p></div>
              </div>
              <div className="px-4 py-2 border-t border-border flex gap-1.5 flex-wrap">
                {table.denominations?.map(d => <span key={d} className={`cms-chip text-[10px] ${CHIP_COLORS[d] || "bg-muted text-foreground"}`}>€{d}</span>)}
              </div>
            </div>
          );
        })}
