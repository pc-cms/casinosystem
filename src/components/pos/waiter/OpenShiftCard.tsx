...
import { useOpenPosShift } from "@/hooks/use-pos-shift";
...
export const OpenShiftCard = ({ casinoId, userId }: Props) => {
  ...
  const handle = async () => {
    ...
    try {
      await open.mutateAsync({ casino_id: casinoId, waiter_user_id: userId, opening_cash: Math.round(n) });
    ...
    <div className="max-w-md mx-auto mt-12 p-6 rounded-md border border-border bg-card space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Enter the cash you have in the POS register at the start of the shift.
        </p>
      </div>
      <FormGrid>
