...
write_root_compose_env() {
  cat > "${CMS_ROOT}/.env" <<'EOF'
# Auto-generated by Casino System installer.
# Lets `docker compose ...` work from /opt/casino-system.
COMPOSE_FILE=deploy/docker-compose.yml
COMPOSE_PROJECT_NAME=deploy
COMPOSE_ENV_FILES=deploy/.env
EOF
  chmod 0644 "${CMS_ROOT}/.env" 2>/dev/null || true
}

fix_local_file_permissions() {
  chmod 0644 "${CMS_ROOT}/.env" 2>/dev/null || true
  if getent group docker >/dev/null 2>&1; then
    chgrp docker .env 2>/dev/null || true
    chmod 0640 .env 2>/dev/null || true
  else
