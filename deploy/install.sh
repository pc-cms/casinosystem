...
if [[ $REPAIR -eq 1 ]]; then
  ...
  ok "Hotfix применён."
  log "Перезапускаю postgrest (для обновления schema cache)..."
  docker compose restart postgrest >/dev/null 2>&1 || warn "postgrest restart не удался (возможно сервис называется иначе)"
