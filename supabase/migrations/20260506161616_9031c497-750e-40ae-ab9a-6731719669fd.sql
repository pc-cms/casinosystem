...
BEGIN
  ...
  FOR r IN
    SELECT e.player_id AS pid, e.kind, e.amount, e.is_cash, e.created_at, e.id FROM (
      ...
      UNION ALL
      SELECT pca.player_id,
             'in'::text         AS kind,
             pca.chip_in::bigint AS amount,
             true               AS is_cash,
             pca.created_at,
             pca.id
...
BEGIN
  ...
  TRUNCATE _tmp_split;
  FOR r IN
    SELECT e.player_id, e.t_table_id AS table_id, e.kind, e.amount, e.is_cash, e.created_at, e.id FROM (
      SELECT t.player_id,
