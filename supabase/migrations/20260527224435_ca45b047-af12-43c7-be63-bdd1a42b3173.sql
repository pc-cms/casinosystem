-- Rename manual cashless column (print-only) to its final name.
ALTER TABLE public.cage_slots_shifts
  RENAME COLUMN cashless_balance_manual TO cashless_final;

-- Canonical Cage Slots balance formula
...
DECLARE
  ...
  v_expenses         bigint := 0;
  v_add_float        bigint := 0;  -- Ace Fill
  v_collection       bigint := 0;
  v_lg_in            bigint := 0;
  v_lg_out           bigint := 0;
  v_cashless_in      bigint := 0;
  v_cashless_out     bigint := 0;
  v_cashless_balance bigint := 0;  -- derived: IN − OUT
  v_cashless_final   bigint := 0;  -- manual, print only
  v_open_cards       int    := 0;
  v_close_cards      int    := 0;
...
BEGIN
  ...
  v_cash_desk_result := v_delta_cash + v_expenses + v_collection + v_lg_out - v_lg_in;
  v_expected         := v_system_result - v_opening_cash_tzs;
  -- Shift Balance = (CDR + Ace Fill) − (System − Opening) − Cards Miss
  v_balance          := (v_cash_desk_result + v_add_float) - v_expected - v_cards_miss;
...
-- Recompute persisted columns to reflect the new formula.
UPDATE public.cage_slots_shifts SET system_shift_result = system_shift_result WHERE true;