...
ALTER TABLE public.cage_slots_shifts
  ADD COLUMN IF NOT EXISTS cashless_balance_manual bigint NOT NULL DEFAULT 0;
...
BEGIN
  ...
  -- Cashless Balance is a manual entry, used only for the printed check.
  v_cashless_balance := COALESCE(s.cashless_balance_manual, 0)::bigint;

  -- Simplified Slots shift balance:
  --   balance = (closing + Ace Fill) − (System − opening) − Cards Miss
  v_expected   := v_system_result - v_opening_cash_tzs;
  v_counted    := v_closing_cash_tzs + v_add_float;
