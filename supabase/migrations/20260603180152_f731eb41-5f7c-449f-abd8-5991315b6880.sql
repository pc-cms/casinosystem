...
BEGIN
  ...
  -- CANONICAL CDR — tips_cd are cage-neutral by design (the collection inflow
  -- and the later payout net to zero in the physical cash count), therefore
  -- tips_cd_payout MUST NOT be added back here. Including it caused tips to
  -- show up as a positive surplus in Shift Balance.
...
-- Recompute cached balance fields on existing closed shifts so the history
-- view reflects the corrected formula immediately.
UPDATE public.cage_slots_shifts s
SET
  cash_desk_result = (public.compute_slots_shift_balance_from_row(s)->>'cash_desk_result')::bigint,
  balance          = (public.compute_slots_shift_balance_from_row(s)->>'balance')::bigint
WHERE status IN ('closed', 'ready_for_review');