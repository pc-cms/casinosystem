...
DROP POLICY IF EXISTS "Super admins read pending servers" ON public.pending_server_registrations;
CREATE POLICY "Super admins read pending servers"
  ON public.pending_server_registrations FOR SELECT
  TO authenticated
  USING (public.has_role(auth.uid(), 'super_admin'));

DROP POLICY IF EXISTS "Super admins update pending servers" ON public.pending_server_registrations;
