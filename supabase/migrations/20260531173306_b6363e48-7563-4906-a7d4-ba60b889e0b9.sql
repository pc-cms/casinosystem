-- Fix build_business_day_snapshot: use business_date_of() for player_stats,
-- cash_counts, and expenses fallback (previously used raw UTC ::date which
-- mis-buckets sessions between 00:00 UTC and 07:00 EAT rollover).
...
BEGIN
  result := jsonb_set(result, '{cash_counts}', COALESCE((
    ...
    WHERE c.casino_id = _casino_id
      AND public.business_date_of(c.created_at) = _business_date
  ), '[]'::jsonb));
  ...
  result := jsonb_set(result, '{expenses}', COALESCE((
    ...
    WHERE e.casino_id = _casino_id
      AND (
        ...
        OR (s.id IS NULL AND COALESCE(e.business_date, public.business_date_of(e.created_at)) = _business_date)
      )
  ), '[]'::jsonb));
  ...
  result := jsonb_set(result, '{player_stats}', COALESCE((
    ...
    WHERE s.casino_id = _casino_id
      AND public.business_date_of(s.started_at) = _business_date
  ), '[]'::jsonb));
  ...
  INTO v_bar_charge
  FROM pos_tabs t
  JOIN pos_shifts ps ON ps.id = t.shift_id
  WHERE t.casino_id = _casino_id
    AND ps.business_date = _business_date
    AND t.closed_at IS NOT NULL;

  SELECT COALESCE(SUM(COALESCE((t.payment_split->>'cogs')::bigint, 0)), 0)
  INTO v_bar_cogs
  FROM pos_tabs t
  JOIN pos_shifts ps ON ps.id = t.shift_id
  WHERE t.casino_id = _casino_id
  ...
  v_bar_pl := v_bar_charge - v_bar_cogs;
  v_net := v_tables + v_slots - v_expenses_total + v_bar_pl;

  result := jsonb_set(result, '{totals}', jsonb_build_object(
    'tables_result', v_tables,
    'chip_miss', v_chip_miss,
    'slots_result', v_slots,
    'cards_miss', v_cards_miss,
    'expenses', v_expenses_total,
    'bar_gross', v_bar_gross,
    'bar_cash', v_bar_cash,
    'bar_card', v_bar_card,
    'bar_comp_house', v_bar_comp_house,
    'bar_comp_player', v_bar_comp_player,
    'bar_charge', v_bar_charge,
    'bar_cogs', v_bar_cogs,
    'bar_pl', v_bar_pl,
    'bar_bills', v_bar_bills,
    'net_result', v_net
  ));
