...
BEGIN
  ...
  -- Expenses are full-day (no shift split for slots): sum all approved slots expenses for this business day.
...
-- Recompute balances for currently open slots shifts and recent shifts so the change is visible immediately.
UPDATE public.cage_slots_shifts
   SET updated_at = now()
 WHERE business_date >= (CURRENT_DATE - INTERVAL '14 days')::date;