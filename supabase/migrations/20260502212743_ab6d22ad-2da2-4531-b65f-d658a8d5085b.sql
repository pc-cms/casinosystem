...
BEGIN
  ...
  -- Three independent slots per table:
  --   C = chipper  (role ends with 'c': ARc, AR1c, AR2c, ...)
  --   I = inspector (role ends with 'i': ARi, BJi, Pi, AR1i, ...)
  --   D = dealer    (everything else: AR, BJ, P, AR1, AR2, ...)
  -- A table can have one of EACH at the same time slot, but not two of the same kind.
  IF v_role ~ 'c$' THEN
    v_slot := 'C';
  ELSIF v_role ~ 'i$' THEN
    v_slot := 'I';
