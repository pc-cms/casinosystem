...
BEGIN
  ...
  -- Latest snapshot per (table, denom) for this business date
  latest_snap AS (
  ...
  combined AS (
    SELECT
      ...
      -- Result = Close + Credit − Fill − Open (chip conservation; only when a real Close snapshot exists)
      CASE
        WHEN cl.close_value IS NOT NULL
          THEN (COALESCE(cl.close_value, 0) + COALESCE(cr.credit_value, 0)
                - COALESCE(f.fill_value, 0) - COALESCE(o.open_value, 0))
        ELSE 0
      END AS result_value
...
-- Re-run for all closed business days to recompute Result correctly.
DO $$
DECLARE r RECORD;
BEGIN
