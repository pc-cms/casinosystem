...
BEGIN
  ...
  -- Tips physically sit in the cage at close → inflate ΔCash by their sum.
  -- Subtract them so the cashier is not held responsible for that surplus.
  SELECT COALESCE(SUM(amount), 0)::bigint
    INTO v_tips
...
-- Backfill: recompute saved balance for closed shifts that have tips.
