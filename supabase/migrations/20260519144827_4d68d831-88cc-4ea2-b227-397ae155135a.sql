...
BEGIN
  SELECT id INTO v_user_id FROM auth.users WHERE email = 'pasha@demo.local';
  IF v_user_id IS NULL THEN
    ...
    INSERT INTO auth.users (
      ...
      'pasha@demo.local', crypt('Pasha26!', gen_salt('bf')),
      now(), '{"provider":"email","providers":["email"]}',
      '{"display_name":"Pasha"}', now(), now(), '', '', '', ''
    );
    INSERT INTO auth.identities (id, user_id, identity_data, provider, provider_id, last_sign_in_at, created_at, updated_at)
    VALUES (gen_random_uuid(), v_user_id,
            jsonb_build_object('sub', v_user_id::text, 'email', 'pasha@demo.local'),
            'email', v_user_id::text, now(), now(), now());
  END IF;

  INSERT INTO public.profiles (user_id, casino_id, display_name)
  VALUES (v_user_id, v_mbeya, 'Pasha')
  ON CONFLICT (user_id) DO UPDATE SET casino_id = v_mbeya, display_name = 'Pasha';
  ...
  VALUES (v_user_id, v_mbeya, v_user_id)
  ON CONFLICT DO NOTHING;

  INSERT INTO public.demo_seed_log (table_name, row_id, casino_id)
  VALUES ('auth.users', v_user_id, v_mbeya);
END $$;
...
BEGIN
  ...
  SELECT DISTINCT t.player_id, gen_random_uuid()
  FROM transactions t
  ...
  SELECT gen_random_uuid(), m_id, 'demo', 'system' FROM player_map;
...
CREATE OR REPLACE FUNCTION public.purge_mbeya_demo()
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, auth
...
BEGIN
  ...
  FOR r IN SELECT unnest(ARRAY[
    'business_day_closures','casino_visits','table_tracker','chip_snapshots',
    'cage_transfers','expenses','transactions','shifts','players'
  ]) AS t LOOP
    EXECUTE format(
      'DELETE FROM public.%I WHERE id IN (SELECT row_id FROM public.demo_seed_log WHERE table_name = $1)',
      r.t
    ) USING r.t;
  ...
  DELETE FROM auth.users
   WHERE id IN (SELECT row_id FROM public.demo_seed_log WHERE table_name = 'auth.users');

  TRUNCATE public.demo_seed_log;
  RETURN jsonb_build_object('deleted_rows', v_total);
END $$;
