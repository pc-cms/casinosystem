-- Cage Slots: remove tips_cd from Shift Balance formula (tips are physically removed from cage and should NOT appear in the balance).
-- Backfill all existing slots shifts to recompute balance.
...
BEGIN
  ...
  -- Tips CD are physically removed from cage and accounted separately — DO NOT add them back into the balance.
...
-- Backfill: recompute balance for ALL cage_slots_shifts using the new formula.
