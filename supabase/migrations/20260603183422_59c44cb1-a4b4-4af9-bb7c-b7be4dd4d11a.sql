...
BEGIN
  ...
  -- CANONICAL CDR (updated 03 Jun 2026):
  -- Tips IN physically sit in closing cash; tips OUT physically reduce it.
  -- To keep Tips neutral on Shift Balance (IN+ / OUT- → 0 if equal),
  -- subtract TipsIN and add TipsOut back. Unpaid tips remain as a debt to
  -- staff tracked in the tips_cd ledger, never inflating cage balance.
  v_cash_desk_result := v_closing_cash_tzs + v_expenses - v_add_float + v_collection
                        + v_lg_out - v_lg_in
                        - v_tips_cd + v_tips_cd_payout;
...
-- Recompute existing shifts so historic balances reflect the new formula
